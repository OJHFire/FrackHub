...
public class ItemRegistration implements ActionListener{
	JFrame frame;
	...
	ItemRegistration(String name, int id) {
		this.userName = name;
		this.userId = id;
		newItem(new JFrame());
	...
	public void addToDatabase(double value, double costPerDay) {
		...
		try {
			...
			PreparedStatement Pstatement = connection.prepareStatement(
					"insert into ItemsV2(Id, UserID, Name, Type, Description, Value, Borrow_Cost) values(seq_item.nextval,?,?,?,?,?,?)");
