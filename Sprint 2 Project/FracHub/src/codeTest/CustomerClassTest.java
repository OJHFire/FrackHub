...
class CustomerClassTest {
	...
	void testSaveCust() {
			...
			try {
			       ...
			       ResultSet rs = stmt.executeQuery("SELECT COUNT(*) AS count FROM memberv2 WHERE email = 'RebelYell@hotmail.com'");  
			       ...
			       stmt2.executeUpdate("DELETE FROM memberv2 WHERE email = 'RebelYell@hotmail.com'");			       
	...
	void testCustSignIn() {
		...
		try {
		       ...
		       stmt.executeUpdate("DELETE FROM memberv2 WHERE email = 'RebelYell@hotmail.com'");			       
