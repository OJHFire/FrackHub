...
/**
 * Contains GUI to input username and password and sign in to the application.
 */
...
public class signInGUI implements ActionListener{
	...
	public signInGUI(JFrame new_frame) {
		...
		signIn1 = new JPanel();
		signIn1.setBorder(BorderFactory.createEmptyBorder(30,120,120,120));
		...
		btnShowPass.setPreferredSize(new Dimension(100,40));
		...
		btnConSignIn.setPreferredSize(new Dimension(100,40));
	...
	public void conSignIn() {
		if (!txtEmail.getText().isEmpty()) {
			if (txtPassword.getPassword().length != 0) {
				...
				admin = admin.adminSignIn(txtEmail.getText(), txtPassword);
				if (!admin.getUsername().equals("")) {
					if (!admin.getUsername().equals("Error")) {
						optionMenuGUI new_panel = new optionMenuGUI();
						new_panel.optionMenu(admin, frame);
					}
					else {
						signIn1.setBorder(BorderFactory.createEmptyBorder(10,120,120,120));
