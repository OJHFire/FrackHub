...
class ItemClassTest {
	...
	void testSaveItem() {
		...
		try {
		       ...
		       ResultSet rs = stmt1.executeQuery("SELECT COUNT(*) AS count FROM items WHERE description = 'Bosch EasyGrassCut 26 Corded - 280W'");  
		       ...
		       stmt2.executeUpdate("DELETE FROM items WHERE description = 'Bosch EasyGrassCut 26 Corded - 280W'");
